...
void Removeten(int k,char ten[]) {
	NodePtr p, q;
	q = new HashNode;
	int b = HashFuntion(k); 	//tìm vị trí trong bảng băm
	p = bucket[b]; //gán giá trị đầu tiên cuả phần từ thứ i trong bảng băm
	while (p != NULL && _strcmpi(p->Key.hoten, ten) != 0)
	{
		q = p;
		p = p->Next;
	}
...
void showDStheodiem(int diem)
{
	int b = HashFuntion(diem);
	HashNode *p;
	for (p = bucket[b]; p != NULL; p = p->Next)
		xuat1sv(p->Key);
}
void xuattheoten(int diem, char ten[])
{
	int b = HashFuntion(diem);//tim2 vi5 tri1 cua bucket
	HashNode *p;
	for (p = bucket[b]; p != NULL; p = p->Next)
	if (_strcmpi(p->Key.hoten, ten) == 0)
		xuat1sv(p->Key);
